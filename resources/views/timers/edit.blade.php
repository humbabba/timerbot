    ...
    <div class="p-4 md:p-8 max-w-4xl">
        ...
        <form method="POST" action="{{ route('timers.update', $timer) }}" data-ajax-save class="bg-timerbot-panel-light rounded-sm p-6"
            ...
            <div class="mb-6">
                ...
                <p class="text-text-muted text-sm mb-4">Configure sound warnings before each {{ old('participant_term', $timer->participant_term) }}'s time expires. Use negative values for warning sounds after expiration.</p>
            ...
            <div class="mb-6">
                <label for="message" class="block mb-2 font-semibold text-timerbot-teal uppercase text-sm tracking-wider" style="font-family: var(--font-display);">{{ ucfirst(old('participant_term', $timer->participant_term)) }} Message</label>
            ...
            <div class="flex flex-col md:flex-row gap-4">
                ...
                <a href="{{ route('timers.show', $timer) }}" class="btn bg-timerbot-panel text-text hover:bg-divider no-underline text-center">
                    Cancel
                </a>
                @if($timer->canRun(auth()->user()))
                    <a href="{{ route('timers.run', $timer) }}" class="btn btn-primary no-underline text-center">
                        Run Timer
                    </a>
                @endif
            </div>
