<x-layouts.app>
    <div class="p-8 max-w-4xl">
        ...
        <form method="POST" action="{{ route('nodes.store') }}" id="node-form" class="bg-cortex-panel-light rounded-xl p-6">
            @csrf
            ...
            <div class="mb-6">
                ...
                >
                    ...
                    @foreach($voiceProfiles as $profile)
                        <option value="{{ $profile->id }}" {{ old('voice_profile_id') == $profile->id ? 'selected' : '' }}>
                            {{ $profile->name }}
                        </option>
                    @endforeach
                </select>
