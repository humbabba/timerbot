                    ...
                    <div class="bg-timerbot-panel-light rounded-sm p-6 space-y-6">
                        @foreach($groupSettings as $setting)
                            <div class="flex flex-col md:flex-row md:items-start gap-4">
                                <label for="settings_{{ $setting->key }}" class="font-semibold text-timerbot-teal md:w-64 uppercase text-sm tracking-wider md:pt-2" style="font-family: var(--font-display);">
                                        ...
                                        <label class="flex items-center gap-3 cursor-pointer">
                                            ...
                                            <div class="relative">
                                                <input
                                                    ...
                                                    class="sr-only peer"
                                                >
                                                <div class="w-12 h-6 border border-subtle rounded-full peer-checked:bg-timerbot-green peer-checked:border-timerbot-green transition-colors"></div>
                                                <div class="absolute left-1 top-1 w-4 h-4 bg-subtle rounded-full peer-checked:bg-timerbot-black peer-checked:translate-x-6 transition-all"></div>
                                            </div>
                                    ...
                                    @elseif($setting->type === 'integer')
                                        <div>
                                            <input
                                                ...
                                                id="settings_{{ $setting->key }}"
                                                value="{{ $setting->value }}"
                                                class="bg-timerbot-panel border border-divider rounded-sm px-4 py-2 text-text w-32"
                                            >
                                    ...
                                    @else
                                        <div>
                                            <input
                                                ...
                                                id="settings_{{ $setting->key }}"
                                                value="{{ $setting->value }}"
                                                class="bg-timerbot-panel border border-divider rounded-sm px-4 py-2 text-text w-full max-w-md"
                                            >
                                            @if($setting->description)
                                                <p class="text-text-muted text-sm mt-2">{{ $setting->description }}</p>
                                            @endif
